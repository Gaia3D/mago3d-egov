...
public class Pagination {
		...
		if (this.lastPage > 10l && this.pageNo <= ((this.lastPage / this.pageListCount) * this.pageListCount)) {
			if (this.lastPage >= (this.startPage + this.pageListCount)) {
				if (this.pageNo % this.pageListCount == 0l) {
					// TODO 다음을 눌렀을때 현재 페이지 + 10 이 아닌 항상 11, 21, 31... 형태로 표시하고
					// 싶을때
	...
	public long getPageListCount() {
		return pageListCount;
	}

	public void setPageListCount(long pageListCount) {
		this.pageListCount = pageListCount;
	}
	...
	public String toString() {
		return "Pagination [totalCount=" + totalCount + ", rowNumber=" + rowNumber + ", pageNo=" + pageNo
				+ ", firstPage=" + firstPage + ", lastPage=" + lastPage + ", startPage=" + startPage + ", endPage="
				+ endPage + ", prePageNo=" + prePageNo + ", nextPageNo=" + nextPageNo + ", existPrePage=" + existPrePage
				+ ", existNextPage=" + existNextPage + ", pageRows=" + pageRows + ", pageListCount=" + pageListCount
				+ ", offset=" + offset + ", uri=" + uri + ", searchParameters=" + searchParameters + ", pageIndex="
				+ pageIndex + "]";
	}
